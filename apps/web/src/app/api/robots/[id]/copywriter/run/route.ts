...
export async function POST(
  ...
  const policyInput: PolicyInput = {
    ...
    requestedAction: "copywriter.run",
    requestedObjective: { type: "copywriter", action: "run" },
  ...
  if (policyOutput.decision === "BLOCK") {
    await appendLedgerEntry(prisma, {
      tenantId,
      module: "copywriter",
      source: "copywriter-run",
      type: "copywriter_gate",
      state: "failed",
      payload: {
        error: {
  ...
  const latestSignalEntry = await latestLedgerEntry(prisma, {
    tenantId,
    robotId: robot.id,
    types: ["fusion", "signal"],
  });
  ...
  const latestRunSignals = latestSignalEntry?.payload ?? latestRun?.result ?? null;
  const latestIdeaResult = latestIdeaEntry?.payload ?? latestIdea?.result ?? null;

  if (!latestRunSignals || !latestIdeaResult) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    const copy = await prisma.robotCopy.create({
    ...
    await appendLedgerEntry(prisma, {
      ...
      lineage: {
        ...
        dependsOnLedgerIds: [
          ...(latestSignalEntry ? [latestSignalEntry.id] : []),
  ...
  try {
    const response = await openai.responses.create({
      ...
      input: JSON.stringify({
        parasite: latestRunSignals,
        ideator: latestIdeaResult,
        config: robot.config,
      }),
      text: {
